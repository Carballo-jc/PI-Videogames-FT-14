...
const videogame = { name: 'Super Mario Bros 2', description: 'a nice description', platforms: 'Wii', released: "2018-01-12T00:00:00.000Z", rating: 3};
let dbId;
const apiId = '4200'
...
describe('Videogames routes', () => {
  ...
  describe('GET /videogames', () => {
    it('should get statusCode = 200 and 102 videogames (100 from api + 2 from db)', async() =>{
     const response = await agent.get('/videogames').expect(200);
     expect(response.body).to.have.lengthOf(102);
    }
    ).timeout(47000);
    it('success from api by id',  async() =>{
      const response =  await agent.get('/videogame/'+apiId).expect(200);
      const game = response.body;
      expect(game.id).to.be.equals(apiId);
     ...
     it('success from db by id', async() =>{
       const response2 = await agent.get(`/videogame/${dbId}`).expect(200);
       const gameDb = response2.body;
       expect(gameDb.id).to.be.equals(`${dbId}`);
